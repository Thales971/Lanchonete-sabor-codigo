...
export const criar = async (req, res) => {
	try {
		...
		if (!pedidoId)
			return res.status(400).json({ erro: "O campo 'pedidoId' é obrigatório." });
		...
		if (!produto.disponivel)
			return res.status(400).json({ erro: 'Não pode adicionar produto com disponivel = false ao pedido.' });
